...
namespace ns3 {
    ...
    std::ostream &operator<<(std::ostream &os, const NodeContainer &intMetrics) {
        os << "[";
    ...
    CustomTracerBoot::GetTypeId() {
        static TypeId tid = TypeId("CustomTracerBoot")
                                .SetParent<ns3::Object>()
                                .AddConstructor<CustomTracerBoot>()
                                .AddAttribute("TraceFilename",
                                              "Filename to save trace",
                                              StringValue("results/dataCustom.dat"),
                                              MakeStringAccessor(&CustomTracerBoot::mTraceFilename),
                                              MakeStringChecker())
                                .AddAttribute("NodesToMonitor",
    ...
    void CustomTracerBoot::doInstall() {
        resetTraceCounters();
        updateCounters();
        ...
        *ostream->GetStream() << "Time(1)"
        ...
        for (auto node : mNodesToMonitor) {
            ...
            Config::Connect(nodeIdConn + "/DeviceList/*/$ns3::WifiNetDevice/Phy/State/State", MakeCallback(&CustomTracerBoot::PhyStateChg, this));
            Config::Connect(nodeIdConn + "/DeviceList/*/$ns3::WifiNetDevice/Phy/State/RxError", MakeCallback(&CustomTracerBoot::PhyRxError, this));
            Config::Connect(nodeIdConn + "/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyTxDrop", MakeCallback(&CustomTracerBoot::PhyTxDrop, this));
            Config::Connect(nodeIdConn + "/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyRxDrop", MakeCallback(&CustomTracerBoot::PhyRxDrop, this));
            Config::ConnectWithoutContext(nodeIdConn + "/ApplicationList/*/$ns3::ndn::Consumer/FirstInterestDataDelay", MakeCallback(&CustomTracerBoot::FirstInterestDataDelay, this));
            Config::ConnectWithoutContext(nodeIdConn + "/ApplicationList/*/$ns3::ndn::Consumer/LastRetransmittedInterestDataDelay", MakeCallback(&CustomTracerBoot::LastRetransmittedInterestDataDelay, this));
    ...
    void CustomTracerBoot::collectData() {
        updateCounters();
    ...
    void CustomTracerBoot::resetTraceCounters() {
    ...
    void CustomTracerBoot::updateCounters() {
    ...
    std::string CustomTracerBoot::decodeCurrentWifiState(const WifiPhyState &state) {
        switch (state) {
    ...
    std::string CustomTracerBoot::decodeWifiRxFailureReason(const WifiPhyRxfailureReason &reason) {
        switch (reason) {
    ...
    void CustomTracerBoot::FirstInterestDataDelay(Ptr<ndn::App> app, uint32_t seqno, Time delay, uint32_t retx_count, int32_t hop_count) {
    ...
    void CustomTracerBoot::LastRetransmittedInterestDataDelay(Ptr<ndn::App> app, uint32_t seqno, Time delay, int32_t hopCount) {
        NFD_LOG_DEBUG("Seq: " << seqno << " - Delay: " << delay.ToDouble(Time::MS) << " - HopCount: " << hopCount);
    ...
    void CustomTracerBoot::PhyStateChg(std::string context, Time start, Time duration, WifiPhyState state) {
        // double currentA       = 0.0;
    ...
    void CustomTracerBoot::PhyRxError(std::string context, Ptr<const Packet> pkt, double snr) {
        // no need to count this dropped packet as
        // it has already been counted in PhyRxDrop
        // NS_LOG_DEBUG("Pkt RX error - SNR: " << snr);
    }

    void CustomTracerBoot::PhyTxDrop(std::string context, Ptr<const Packet> pkt) {
        NS_LOG_DEBUG("Pkt TX drop");
        phyTxDropped++;
    }

    void CustomTracerBoot::PhyRxDrop(std::string context, Ptr<const Packet> pk, WifiPhyRxfailureReason reason) {
