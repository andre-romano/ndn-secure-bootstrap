...
namespace ns3 {
    ...
    TypeId CustomConsumer::GetTypeId() {
        static TypeId tid =
            TypeId("CustomConsumer")
                ...
                .AddAttribute(
                    "Prefix", "Name of the Interest", StringValue("/"),
                    MakeStringAccessor(&CustomConsumer::m_interestName),
                    MakeStringChecker()
                )
                .AddAttribute(
                    "Frequency", "Frequency of interest packets",
                    StringValue("1.0"),
                    MakeDoubleAccessor(&CustomConsumer::m_frequency),
                    MakeDoubleChecker<double>()
                )
                .AddAttribute(
                    "LifeTime", "Lifetime of interest packets (in seconds)",
                    StringValue("2s"),
                    MakeTimeAccessor(&CustomConsumer::m_interestLifeTime),
                    MakeTimeChecker()
                )
                .AddAttribute(
                    "Randomize",
                    ...
                    StringValue("none"),
                    MakeStringAccessor(
                        &CustomConsumer::SetRandomize,
                        &CustomConsumer::GetRandomize
                    ),
                    MakeStringChecker()
                )
                .AddAttribute(
                    "ValidatorConf", "Validator config filename",
                    StringValue("./scratch/sim_bootsec/config/validator.conf"),
                    MakeStringAccessor(&CustomConsumer::m_validatorFilename),
                    MakeStringChecker()
                );
        return tid;
    }
    ...
    void CustomConsumer::StartApplication() {
        ...
        // create ndn::Face to allow real-world application to interact inside
        // ns3
        ...
        // Add entry to FIB for `/prefix/sub` (we are the Producer for this
        // content) ndn::FibHelper::AddRoute(GetNode(), "/prefix/sub", m_face,
        // 0);
    ...
    void CustomConsumer::SetupValidator() {
        ...
        try {
            ...
            throw std::runtime_error(
                "Failed to load validation rules file=" + m_validatorFilename +
                " Error=" + e.what()
            );
        }
    }

    void CustomConsumer::ScheduleNextPacket() {
        if(m_firstTime) {
            m_sendEvent = Simulator::Schedule(
                Seconds(0.0), &CustomConsumer::SendInterest, this
            );
            m_firstTime = false;
        } else if(!m_sendEvent.IsRunning()) {
            m_sendEvent = Simulator::Schedule(
                (m_random == 0) ? Seconds(1.0 / m_frequency)
                                : Seconds(m_random->GetValue()),
                &CustomConsumer::SendInterest, this
            );
        }
    }

    void CustomConsumer::SendInterest() {
        /////////////////////////////////////
        ...
        interest->setNonce(
            rand->GetValue(0, std::numeric_limits<uint32_t>::max())
        );
        interest->setInterestLifetime(
            ndn::time::milliseconds(m_interestLifeTime.GetMilliSeconds())
        );
    ...
    void CustomConsumer::OnData(std::shared_ptr<const ndn::Data> data) {
        NS_LOG_DEBUG(
            "Receiving Data packet: " << data->getName() << " - KeyLocator: "
                                      << data->getSignature().getKeyLocator()
        );
        m_validator->validate(
            *data, MakeCallback(&CustomConsumer::OnDataValidated, this),
            MakeCallback(&CustomConsumer::OnDataValidationFailed, this)
        );
        // std::cout << "DATA received for name " << data->getName() <<
        // std::endl;
    }
    ...
    void CustomConsumer::OnDataValidationFailed(
        const ndn::Data &data, const ::ndn::security::v2::ValidationError &error
    ) {
        NS_LOG_ERROR(
            "Validation FAILED for Data packet: " << data.getName()
                                                  << " - Error: " << error
        );
    }
    ...
    void CustomConsumer::SetRandomize(const std::string &value) {
        if(value == "uniform") {
            ...
            m_random->SetAttribute(
                "Bound", DoubleValue(50 * 1.0 / m_frequency)
            );
        } else
            m_random = 0;
