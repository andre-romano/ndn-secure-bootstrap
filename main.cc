...
namespace ns3 {
    ...
    std::string constructFaceUri(Ptr<NetDevice> netDevice) {
        ...
        NS_LOG_DEBUG(
            "constructFaceUri( netDev = " << netDevice << ") - URI = " << uri
        );

        return uri;
    }

    shared_ptr<nfd::face::Face> addFaceWifiNetDeviceCallback(
        Ptr<Node> node, Ptr<ndn::L3Protocol> ndn, Ptr<NetDevice> netDevice
    ) {
        ...
        auto transport = make_unique<ndn::NetDeviceTransport>(
            ...
            "netdev://[ff:ff:ff:ff:ff:ff]", ::ndn::nfd::FACE_SCOPE_NON_LOCAL,
            ::ndn::nfd::FACE_PERSISTENCY_PERSISTENT, linkType
        );

        auto face = std::make_shared<nfd::face::Face>(
    ...
    int main(int argc, char *argv[]) {
        ...
        Config::SetDefault(
            "ns3::WifiRemoteStationManager::NonUnicastMode", wifiRate
        );
        ...
        std::string nValidatorConf =
            "scratch/sim_bootsec/config/validator.conf";
        std::string nTrustAnchorCert =
            "scratch/sim_bootsec/config/trustanchor.cert";
        ...
        cmd.AddValue(
            "nValidatorConf", "Validator config filename", nValidatorConf
        );
        cmd.AddValue(
            "nTrustAnchorCert", "Trust anchor certificate filename",
            nTrustAnchorCert
        );
        cmd.AddValue(
            "nInitialEnergy", "Initial energy of the nodes", nInitialEnergy
        );
        ...
        wifi.SetRemoteStationManager(
            "ns3::ConstantRateWifiManager", "DataMode", wifiRate
        );
        ...
        mobility.SetPositionAllocator(
            "ns3::RandomDiscPositionAllocator", // random position based on
                                                // polar radius
            "X",
            StringValue(to_string(MAP_SIZE / 2)), // center in x axis of the map
            "Y",
            StringValue(to_string(MAP_SIZE / 2)), // center in y axis of the map
            "Rho",
            StringValue(
                "ns3::UniformRandomVariable[Min=0|Max=" +
                to_string(MAP_SIZE / 2) + "]"
            )
        ...
        mobility.SetMobilityModel(
            ...
            ), // speed will vary between min & max
            "Bounds",
            StringValue(
                "0|" + to_string(MAP_SIZE) + "|0|" + to_string(MAP_SIZE)
            )
        );
        ...
        basicSourceHelper.Set(
            "BasicEnergySourceInitialEnergyJ", DoubleValue(nInitialEnergy)
        );
        /* device energy model */
        ...
        ndnHelper.AddFaceCreateCallback(
            ...
            MakeCallback(&addFaceWifiNetDeviceCallback)
        );
        ...
        NetDeviceContainer wifiNetDevices =
            wifi.Install(wifiPhyHelper, wifiMacHelper, nodes);
        ...
        DeviceEnergyModelContainer deviceModels =
            radioEnergyHelper.Install(fwdNetDevices, sources);
        ...
        // ndn::StrategyChoiceHelper::Install(producers, "/",
        // "/localhost/nfd/strategy/multicast");
        ndn::StrategyChoiceHelper::Install(
            consumers, "/", "/localhost/nfd/strategy/multicast"
        );
        ndn::StrategyChoiceHelper::Install(
            forwarders, "/", "/localhost/nfd/strategy/multicast"
        );
        ndn::StrategyChoiceHelper::Install(
            producers, "/", "/localhost/nfd/strategy/multicast"
        );

        // 6.0 Set up Trust Anchor
        auto rootIdentityPrefix = "/test";
        NS_LOG_INFO("Installing Trust Anchor ...");
        ::ndn::security::v2::KeyChain rootKeyChain(
            "pib-memory:", "tpm-memory:"
        );
        auto rootIdentity = rootKeyChain.createIdentity(rootIdentityPrefix);
        ::ndn::io::save(
            rootIdentity.getDefaultKey().getDefaultCertificate(),
            nTrustAnchorCert
        );
        NS_LOG_INFO("Root Identity: " << rootIdentity);
        NS_LOG_INFO(
            "Root Key Type: " << rootIdentity.getDefaultKey().getKeyType()
        );
        ...
        consumerHelper.SetAttribute(
            "Frequency", DoubleValue(nPktFreq)
        ); // consumer CBR interest/second
        consumerHelper.SetAttribute(
            "Randomize", StringValue("uniform")
        ); // consumer randomize send time
        consumerHelper.SetAttribute(
            "ValidatorConf", StringValue(nValidatorConf)
        );
        consumerHelper.SetPrefix(prefixName);
        // set random start time
        for(uint32_t i = 0; i < nConsumers; i++) {
            Ptr<UniformRandomVariable> start_time =
                CreateObject<UniformRandomVariable>();
            consumerHelper.Install(consumers[i])
                .Start(Seconds(start_time->GetValue(0.2, 0.75))
                ); // consumer start time
        }
        ...
        producerHelper.SetAttribute(
            "Freshness", TimeValue(Seconds(nFreshness))
        );
        producerHelper.SetAttribute(
            "PayloadSize", StringValue("1480")
        );                                    // payload MTU
        ...
        for(auto it = producersApps.Begin(); it != producersApps.End(); it++) {
            ...
            if(!customProducerApp)
                continue;
            customProducerApp->setSignCallback(
                [&](::ndn::security::v2::Certificate cert) {
                    ...
                    rootKeyChain.sign(
                        cert, ::ndn::security::SigningInfo(rootIdentity)
        ...
        customTracer->SetAttribute(
            "TraceFilename", StringValue("results/dataCustomCons.dat")
        );
        customTracer->SetAttribute(
            "NodesToMonitor", NodeContainerValue(consumers)
        );

        auto customTracerFwd = CreateObject<CustomTracer>();
        customTracerFwd->SetAttribute(
            "TraceFilename", StringValue("results/dataCustomFwd.dat")
        );
        customTracerFwd->SetAttribute(
            "NodesToMonitor", NodeContainerValue(forwarders)
        );

        auto customTracerProd = CreateObject<CustomTracer>();
        customTracerProd->SetAttribute(
            "TraceFilename", StringValue("results/dataCustomProd.dat")
        );
        customTracerProd->SetAttribute(
            "NodesToMonitor", NodeContainerValue(producers)
        );
