...
NS_LOG_COMPONENT_DEFINE("CustomTracer");
...
namespace ns3 {
    ...
    NS_OBJECT_ENSURE_REGISTERED(CustomTracer);

    // register NS-3 type
    TypeId CustomTracer::GetTypeId() {
        static TypeId tid =
            TypeId("CustomTracer")
                .SetParent<ns3::Object>()
                .AddConstructor<CustomTracer>()
                .AddAttribute(
                    "TraceFilename", "Filename to save trace",
                    StringValue("results/dataCustom.dat"),
                    MakeStringAccessor(&CustomTracer::mTraceFilename),
                    MakeStringChecker()
                )
                .AddAttribute(
                    "NodesToMonitor", "Nodes to keep track of",
                    NodeContainerValue(),
                    MakeNodeContainerAccessor(&CustomTracer::mNodesToMonitor),
    ...
    CustomTracer::CustomTracer() {
        Simulator::Schedule(Seconds(0), &CustomTracer::doInstall, this);
    }

    void CustomTracer::doInstall() {
        resetTraceCounters();
        updateCounters();
        ...
        *ostream->GetStream() << "Time(1)"
        ...
        for(auto node : mNodesToMonitor) {
            ...
            Config::Connect(
                ...
                MakeCallback(&CustomTracer::PhyStateChg, this)
            );
            Config::Connect(
                nodeIdConn +
                    "/DeviceList/*/$ns3::WifiNetDevice/Phy/State/RxError",
                MakeCallback(&CustomTracer::PhyRxError, this)
            );
            Config::Connect(
                nodeIdConn + "/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyTxDrop",
                MakeCallback(&CustomTracer::PhyTxDrop, this)
            );
            Config::Connect(
                nodeIdConn + "/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyRxDrop",
                MakeCallback(&CustomTracer::PhyRxDrop, this)
            );
            Config::ConnectWithoutContext(
                nodeIdConn + "/ApplicationList/*/$ns3::ndn::Consumer/"
                             "FirstInterestDataDelay",
                MakeCallback(&CustomTracer::FirstInterestDataDelay, this)
            );
            Config::ConnectWithoutContext(
                nodeIdConn + "/ApplicationList/*/$ns3::ndn::Consumer/"
                             "LastRetransmittedInterestDataDelay",
                MakeCallback(
                    &CustomTracer::LastRetransmittedInterestDataDelay, this
                )
            );

            // monitor NDN delay and hop count - NOT PRECISE, BEST OT USE
            // FIRSTINTERESTDATADELAY from consumers auto l3         =
            // node->GetObject<ns3::ndn::L3Protocol>(); auto ndnFwd     =
            // l3->getForwarder(); calculate delay & hopCount
            // ndnFwd->beforeSatisfyInterest.connect([&delayStr,
            // &hopCountStr](const nfd::pit::Entry &pitEntry, const
            // nfd::face::Face &ingressFace, const ndn::Data &data) {
            //     for (auto iter = pitEntry.out_begin(); iter !=
            //     pitEntry.out_end(); iter++) {
            //         auto &outRecord = *iter;
            //         if (outRecord.getFace().getId() != ingressFace.getId())
            //         continue;
            //         // calcualte delay
            //         double currDelayMs =
            //         ndn::time::duration_cast<ndn::time::nanoseconds>(ndn::time::steady_clock::now()
            //         - outRecord.getLastRenewed()).count() / 1e6;
    ...
    void CustomTracer::collectData() {
        ...
        for(auto node : mNodesToMonitor) {
            ...
            for(auto &face : faceTable) {
                ...
                droppedInt.current +=
                    linkService->getCounters().nDroppedInterests;
            }
        ...
        Simulator::Schedule(Seconds(1), &CustomTracer::collectData, this);
    }

    void CustomTracer::saveDataToFile() {
        *ostream->GetStream()
            << setprecision(2) << fixed // setprecision(6) << defaultfloat
            ...
            << energyConsumption << "\t" << initialEnergy << "\t"
            << phyRxDropped << "\t" << phyTxDropped << "\t"
            << firstDelayStr.get() << "\t" << lastDelayStr.get() << "\t"
            << hopCountStr.get() << "\t" << droppedInt.getDiff() << "\t"
            << retxInt << "\t" << uniqData << "\n";
    }

    void CustomTracer::resetTraceCounters() {
    ...
    void CustomTracer::updateCounters() {
    ...
    std::string CustomTracer::decodeCurrentWifiState(const WifiPhyState &state
    ) {
        switch(state) {
    ...
    std::string
    CustomTracer::decodeWifiRxFailureReason(const WifiPhyRxfailureReason &reason
    ) {
        switch(reason) {
    ...
    void CustomTracer::FirstInterestDataDelay(
        Ptr<ndn::App> app, uint32_t seqno, Time delay, uint32_t retx_count,
        int32_t hop_count
    ) {
        // NFD_LOG_DEBUG("Seq: " << seqno << " - Delay: " <<
        // delay.ToDouble(Time::MS) << " - RetxCount: " << retx_count << " -
        // HopCount: " << hop_count);
    ...
    void CustomTracer::LastRetransmittedInterestDataDelay(
        Ptr<ndn::App> app, uint32_t seqno, Time delay, int32_t hopCount
    ) {
        NFD_LOG_DEBUG(
            "Seq: " << seqno << " - Delay: " << delay.ToDouble(Time::MS)
                    << " - HopCount: " << hopCount
        );
    ...
    void CustomTracer::PhyStateChg(
        ...
        // auto energySrcCont    =
        // node->GetObject<ns3::EnergySourceContainer>(); if (energySrcCont !=
        // NULL) {
        //     auto energySrc = energySrcCont->Get(0);
        //     if (energySrc != NULL) {
        //         consEnergySrc        = energySrc->GetInitialEnergy() -
        //         energySrc->GetRemainingEnergy(); auto energyModelCont =
        //         energySrc->FindDeviceEnergyModels("ns3::WifiRadioEnergyModel");
        //         auto wifiEnergy      = (energyModelCont.GetN() > 0 ?
        //         energyModelCont.Get(0)->GetObject<ns3::WifiRadioEnergyModel>()
        //         : NULL); if (wifiEnergy != NULL) {
        //             currentA       = wifiEnergy->GetCurrentA();
        //             currState      =
        //             decodeCurrentWifiState(wifiEnergy->GetCurrentState());
    ...
    void CustomTracer::PhyRxError(
        std::string context, Ptr<const Packet> pkt, double snr
    ) {
    ...
    void CustomTracer::PhyTxDrop(std::string context, Ptr<const Packet> pkt) {
        NS_LOG_DEBUG("Pkt TX drop");
        phyTxDropped++;
    }

    void CustomTracer::PhyRxDrop(
        std::string context, Ptr<const Packet> pk, WifiPhyRxfailureReason reason
    ) {
        NS_LOG_DEBUG(
            "Pkt RX drop - Reason: " << decodeWifiRxFailureReason(reason)
        );
        phyRxDropped++;
    }

    // static void CourseChange(std::string context, Ptr<const MobilityModel>
    // mobility) {
    //     Vector pos = mobility->GetPosition();
    //     Vector vel = mobility->GetVelocity();
    //     std::cout << Simulator::Now()
    //               << " RandomDirection2dMobilityModel:CourseChange(): MODEL="
    //               << mobility
    ...
    // power is in dBi
    // void powerChange(Ptr<OutputStreamWrapper> stream, std::string context,
    // double oldPower, double newPower, Mac48Address remoteAddress){
    //     NS_LOG_UNCOND(Simulator::Now().GetSeconds() << "\t" << newPower);
    //     *stream->GetStream() << Simulator::Now().GetSeconds() << "\t" <<
    //     oldPower << "\t" << newPower << std::endl;
    // }
    ...
    //     node->GetId() << " remainingEnergy(): Current remaining energy = " <<
    //     remainingEnergy << "J"); NS_LOG_UNCOND("+" <<
    //     Simulator::Now().GetSeconds() << "s  remainingEnergy(): Current
    ...
    //                       << "s Total energy consumed by radio = " <<
    //                       totalEnergy << "J");
    // }
